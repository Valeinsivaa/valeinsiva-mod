...
module.exports = {
    ...
    execute: async (client, message, args, beş_embed) => {
        ...
        if (message.member.permissions.has(PermissionFlagsBits.Administrator)) {
            ...
            message.reply({ embeds: [beş_embed.setDescription(`> **Başarıyla Kullanıcıyı Kanalınıza Çektiniz!**`).setColor("#00ff00")] }).sil(5);
            client.true(message)
        } else {
            mesajbeş = message.reply({ content: `> **${member}, ${message.author} \`${member.voice.channel.name}\` Adlı Kanala Sizi Çekmek İstiyor, Kabul Ediyomusun?**`, components: [beş_dropdown] })
