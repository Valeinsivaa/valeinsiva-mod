...
module.exports = {
    ...
    execute: async (client, message, args, beş_embed) => {
        ...
        if(!member) return message.reply({ embeds: [beş_embed.setDescription(`> **Geçerli Bir Kullancı Belirt!**`)] }).sil(5);
     ...
     if(member.user.bot) return message.reply({ embeds: [beş_embed.setDescription(`> **Bir Bot'a İşlem Uygulayamazsın!**`)]}).sil(5);
        ...
        if (member.roles.cache.has(jailRoles[0]))return message.reply({ embeds: [beş_embed.setDescription(`> **Belirtilen Kullanıcı Zaten Cezalıda!**`)]}).sil(5);
        if(!member.manageable)return message.reply({ embeds: [beş_embed.setDescription(`> **Belirtilen Kullanıcıya İşlem Yapmaya Yetkim Yetmiyor!**`)]}).sil(5);
