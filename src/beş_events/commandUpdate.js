...
module.exports = async (oldMessage,newMessage) => {
    ...
    .setFooter({ text: beş_config.footer ? beş_config.footer : `Valeinsiva Was Here`, iconURL: newMessage.author.avatarURL({ dynamic: true, size: 2048 }) })
    if (cmd) {
    cmd.execute(client, newMessage, args, beş_embed);
    if(client.kanalbul("commandlog")){
        client.kanalbul("commandlog").send({
